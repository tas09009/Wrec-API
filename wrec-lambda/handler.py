...
def handler(event, context):
    ...
    with db_engine.connect() as connection:
        ...
        s3_client = boto3.client("s3")  # stay in the function?
        ...
        return {"statusCode": 200, "body": "Processed books successfully"}
...
def parse_csv_data(csv_reader):
    ...
    for row in csv_reader:
        ...
        author = (
            unicodedata.normalize("NFD", row[AUTHOR_IDX])
            .encode("ascii", "ignore")
            .decode("utf-8")
        )
        title = (
            unicodedata.normalize("NFD", row[TITLE_IDX])
            .encode("ascii", "ignore")
            .decode("utf-8")
        )
...
def process_book_data(book_data, connection):
    for book in book_data:
        result = connection.execute(
            sa.text("SELECT * FROM books WHERE title = :title"),
            {"title": book["title"]},
        )
